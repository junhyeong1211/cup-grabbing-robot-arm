...
1.  **로봇 모델 교체 (KUKA → Franka Panda):**
    * **문제:** 기존 `kuka_iiwa/model.urdf` 모델에는 제어 가능한 그리퍼가 없다는 치명적인 문제를 발견했다.
...
## 4. '일반화'를 향하여 : 커리큘럼 학습 Level 1 (랜덤 위치)

### 가설
'고정된 컵 잡기'를 마스터한 AI에게, 약간의 '변수'를 주어 **'일반화' 능력**을 가르칠 수 있을 것이다. 즉, AI가 특정 위치를 암기한 것이 아니라, **"컵의 위치를 보고 찾아가는"** 능력을 학습했는지 검증한다.

### 적용된 수정 사항
1.  **컵 위치 무작위화:** `reset` 함수에서 컵의 시작 위치를 고정 값 대신, `np.random.uniform`을 사용하여 **좁은 범위 내에서 무작위로 생성**하도록 변경했다.
2.  **추가 훈련 :** 처음부터 새로 훈련하는 대신, L1에서 성공적으로 훈련된 모델을 `PPO.load()`로 불러와, 바뀐 환경에 적응하도록 추가 훈련을 진행했다.

### 결과 및 분석
* **훈련 과정:** 초기에는 새로운 무작위 위치에 약간 혼란을 겪었지만, 기존의 학습 경험 덕분에 훨씬 빠르게 높은 평균 보상(`ep_rew_mean`)에 도달했다. '추가 훈련'의 효율성을 확인할 수 있었다.
* **테스트:** 최종 훈련된 L2 모델은 **매번 위치가 바뀌는 컵을 일관되게 찾아 잡고 들어 올리는 데 성공**했다!
* **성공 GIF (랜덤 위치):**
    ![L1 성공 GIF](../cup_grasping_L1_RP_success.gif)

### 깨달음
* AI에게 처음부터 너무 어려운 문제를 주는 것보다, 쉬운 문제부터 시작해 점진적으로 난이도를 높이는 **'커리큘럼 학습'**이 매우 효과적임을 다시 한번 확인했다.
* `Fine-tuning`을 통해 기존의 지식을 바탕으로 새로운 문제에 더 빠르게 적응시킬 수 있음을 직접 경험했다.

---

## 5. '강인함'을 향하여 : 도메인 랜덤화 Level 2 (시각적 교란)

### 가설
Sim-to-Real 문제의 핵심인 '리얼리티 갭'을 극복하기 위해, "Domain Randomization" 논문의 아이디어를 적용한다. 시각적 요소를 무작위로 바꾸는 혼란스러운 환경에서 AI를 훈련시키면, AI가 색상이나 조명 같은 비본질적인 정보가 아닌 '형태'라는 본질에만 집중하게 되어 더 **강인한 일반화 능력**을 갖게 될 것이다.

### 적용된 수정 사항
`reset` 함수가 호출될 때마다, `PyBullet`의 API를 이용해 아래 요소들을 무작위로 변경하는 코드를 추가했다.
1.  **조명 랜덤화:** `p.configureDebugVisualizer`를 사용하여 조명의 방향을 무작위로 설정.
2.  **색상 랜덤화:** `p.changeVisualShape`를 사용하여 바닥, 로봇, 컵의 RGBA 색상 값을 무작위로 설정.

### 결과 및 분석
* **훈련 과정:** 예상대로, 이전보다 훨씬 더 어려운 문제였기 때문에 훈련 초반에는 학습 속도가 더뎠다. 하지만 충분한 훈련 시간(약 50만 스텝 이상)이 주어지자, AI는 시각적 혼란 속에서도 점차 컵을 찾아내는 방법을 터득하기 시작했다.
* **테스트:** 최종 훈련된 L3 모델은 **바닥과 컵의 색깔이 매번 알록달록하게 바뀌는 와중에도** 일관되게 컵을 잡고 들어 올리는 데 성공했다!
* **성공 GIF (도메인 랜덤화 환경):**
    ![L2 성공 GIF](../cup_grasping_L2_DR_success.gif)

### 깨달음
* Sim-to-Real 문제에서 시뮬레이션의 '사실성'보다 **'다양성'** 이 더 중요할 수 있다는 논문의 주장을 직접 증명했다.
* AI가 특정 시각적 패턴을 암기하는 것을 넘어, 문제의 '본질'을 학습하도록 유도하는 환경 설계의 중요성을 깨달았다.

## 6. 최종 성공 및 결론: AI를 '가르친다'는 것의 의미
위와 같은 복합적인 개선과 **50만 번 이상의 충분한 훈련** 끝에, AI는 마침내 **무작위 위치에 생성된 컵을 스스로 찾아 잡고 들어 올리는 데 성공**했다. 하지만 이 성공은 한 번에 오지 않았다. 오히려 수많은 실패와 그 원인을 분석하는 과정 속에서 얻어낸 값진 결과물이었다.
